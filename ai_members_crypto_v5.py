    ...
    if data.get("hist") is not None:
        ...
        with tab4:
            ...
            if market_mode == "Crypto":
                ...
                selected_rank = st.selectbox("Crypto 排行榜條件" if lang == "zh" else "Crypto ranking mode", crypto_options, key="crypto_rank_mode")
                crypto_rank_df = build_market_scan_table(market_mode="Crypto", lang=lang, limit=max(scan_limit, 40), scan_mode=scan_mode)
                crypto_rank_view = apply_screener_option(crypto_rank_df, selected_rank, market_mode="Crypto", lang=lang)
                ...
                selected_rank = st.selectbox("股票排行榜條件" if lang == "zh" else "Stock ranking mode", stock_options, key="stock_rank_mode")
                stock_rank_df = build_market_scan_table(market_mode="Stock", lang=lang, limit=max(scan_limit, 40), scan_mode=scan_mode)
                stock_rank_view = apply_screener_option(stock_rank_df, selected_rank, market_mode="Stock", lang=lang)
        ...
        with tab5:
            ...
            if market_mode == "Crypto":
                ...
                selected_screen = st.selectbox("Crypto 選股條件" if lang == "zh" else "Crypto screener option", crypto_options, key="crypto_screen_option")
                crypto_df = build_market_scan_table(market_mode="Crypto", lang=lang, limit=max(scan_limit, 40), scan_mode=scan_mode)
                crypto_filtered = apply_screener_option(crypto_df, selected_screen, market_mode="Crypto", lang=lang)
                ...
                selected_screen = st.selectbox("股票選股條件" if lang == "zh" else "Stock screener option", stock_options, key="stock_screen_option")
                stock_df = build_market_scan_table(market_mode="Stock", lang=lang, limit=max(scan_limit, 40), scan_mode=scan_mode)
                stock_filtered = apply_screener_option(stock_df, selected_screen, market_mode="Stock", lang=lang)
